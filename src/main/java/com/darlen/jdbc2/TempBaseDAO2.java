...
public class TempBaseDAO2<T> {
     ...
     * 保存
     */
    public void save(T entity,String tableName) throws Exception{
        String realTableName = "".equals(Util.nullToString(tableName)) ?
                entity.getClass().getSimpleName().toLowerCase() : tableName.toLowerCase();
        //SQL语句,insert into table name (
        String sql = "insert into " + realTableName + "(";
        ...
        while(iter.hasNext()) {
            ...
            if(method.getReturnType().getSimpleName().indexOf("String") != -1) {
                ...
                statement.setDate(++i, new java.sql.Date(this.getDate(method, entity).getTime()));
    ...
    /**
     * 方法返回类型为String时,返回的SQL语句拼装值.比如'abc',对应get  **根据方法名去调用实体类
     */
